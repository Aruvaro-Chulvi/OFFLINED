...
|---|---|---|---|
| kiwix-serve (kiwix-tools) | 3.7.0 | GPL-3.0 | `LICENSE_kiwix-tools_kiwix-serve_GPL-3.0.txt` — fuente: `Offlined Data\Wikis\source\` y `Offlined Data\Wikis\KIWIX-SOURCE.txt` |
...
| FFmpeg (vía paquete Python `imageio_ffmpeg`, conversión de audio en Notas y Generador de voz) | 7.1-essentials (build gyan.dev, `--enable-gpl --enable-version3`) | GPL-3.0 | `LICENSE_FFmpeg_GPL-3.0.txt` — se invoca como proceso externo vía `subprocess.run` (mera agregación); fuente y detalles: `FFMPEG-SOURCE.txt` |
...
**Nota LGPL (pystray, GEOS, python-soxr, libsndfile):** se usan sin
modificaciones como librerías importadas dinámicamente. La LGPL no afecta
a la licencia de Offlined, pero exige incluir su texto (hecho) y no
impedir que el usuario sustituya la librería. GEOS viene incrustada dentro
de los wheels de Shapely (`geos_c-*.dll`) y se usa para generar los tiles
MVT de las cartas náuticas (sección Mares y Océanos, endpoint
`/api/maps/seamarks/tiles`). python-soxr y libsndfile (esta última
incrustada en el wheel de `soundfile` como `libsndfile-1.dll`) son
dependencias de audio del motor de voz OmniVoice — ver
`LICENSE_soxr_LGPL-2.1.txt`.

### Dependencias del motor de voz OmniVoice (síntesis y clonación de voz)

`omnivoice` se empaqueta con `collect_all('omnivoice')` en el `.spec`, lo
que arrastra las siguientes librerías Python a la distribución. Todas son
de licencia permisiva (sin obligación de código fuente):

| Componente | Licencia | Archivo |
|---|---|---|
| transformers | Apache-2.0 | `LICENSE_Apache-2.0.STUB.txt` |
| accelerate | Apache-2.0 | `LICENSE_Apache-2.0.STUB.txt` |
| tokenizers | Apache-2.0 | `LICENSE_Apache-2.0.STUB.txt` |
| safetensors | Apache-2.0 | `LICENSE_Apache-2.0.STUB.txt` |
| huggingface_hub | Apache-2.0 | `LICENSE_Apache-2.0.STUB.txt` |
| hf_xet | Apache-2.0 | `LICENSE_Apache-2.0.STUB.txt` |
| msgpack | Apache-2.0 | `LICENSE_Apache-2.0.STUB.txt` |
| diskcache | Apache-2.0 | `LICENSE_Apache-2.0.STUB.txt` |
| librosa | ISC | `LICENSE_ISC.STUB.txt` |
| soundfile (wrapper Python) | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| python-soxr | LGPL-2.1-or-later | `LICENSE_soxr_LGPL-2.1.txt` |
| libsndfile (empaquetada por soundfile) | LGPL-2.1 | `LICENSE_soxr_LGPL-2.1.txt` |
| pydub | MIT | `LICENSE_MIT.txt` |
| audioread | MIT | `LICENSE_MIT.txt` |
| einops | MIT | `LICENSE_MIT.txt` |
| rich | MIT | `LICENSE_MIT.txt` |
| typer | MIT | `LICENSE_MIT.txt` |
| watchfiles | MIT | `LICENSE_MIT.txt` |
| httptools | MIT | `LICENSE_MIT.txt` |
| cffi | MIT | `LICENSE_MIT.txt` |
| filelock | MIT | `LICENSE_MIT.txt` |
| narwhals | MIT | `LICENSE_MIT.txt` |
| numba | BSD-2-Clause | `LICENSE_BSD-3-Clause.txt` (texto BSD genérico, válido también para la variante 2-cláusulas) |
| llvmlite | BSD-2-Clause | `LICENSE_BSD-3-Clause.txt` |
| scikit-learn | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| scipy | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| sympy | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| networkx | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| pypdf | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| click | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| python-dotenv | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| fsspec | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| colorama | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| pooch | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| joblib | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| threadpoolctl | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |
| websockets | BSD-3-Clause | `LICENSE_BSD-3-Clause.txt` |

Nota: `PyInstaller`, `altgraph` y `pefile` son herramientas de compilación
(no se distribuyen en el `.exe` final) y no requieren entrada aquí.
`openai_harmony` está instalado en el venv pero no se importa en ningún
punto del código (el manejo del formato "Harmony" de GPT-OSS es un parser
propio en `main.py`) → no se empaqueta, no requiere entrada.
