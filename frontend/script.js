...
let state = {
  ...
  // 📚 Cache de existencia de artículos: { "zimId|Title_Enc": boolean } (true = existe y NO es desambiguación)
  wikiExist: {},

  // 🏷️ Cache de título canónico: { "zimId|norm(title)": "Título_Exacto" } (no guarda desambiguaciones)
  wikiCanon: {},
...
function pickGreeting(agent){
  ...
  if (gg){
    ...
    if (typeof gg === "object") {
      ...
      if (Array.isArray(v2) && val2.length) return v2[Math.floor(Math.random()*v2.length)];
      if (typeof v2 === "string" && v2.trim()) return v2.trim();
    }
  }

  // 3) fallback elegante si no hay greeting en el perfil
  const prof = L(agent.profession) || "";
  if (lang === "es") {
    return `¡Hola! Soy ${agent.name}${prof ? ` (${prof})` : ""}. ¿En qué puedo ayudarte?`;
  } else if (lang === "fr") {
    return `Bonjour ! Je suis ${agent.name}${prof ? ` (${prof})` : ""}. Comment puis-je aider ?`;
  }
  return `Hi! I'm ${agent.name}${prof ? ` (${prof})` : ""}. How can I help?`;
}
...
function init(){
  ...
  showBackendModel().then(async ()=>{
    ...
    // Pinta la vista guardada (llm/agents/wiki)
...
function attachEvents(){
  ...
  // 🆕 Chips del historial de agentes recientes
  document.addEventListener('click', async (e)=>{
    const chip = e.target.closest && e.target.closest('.recent-chip');
    if (chip){
      e.preventDefault();
...
// ===== WIKI LINKING CORE =====
// Nota: mantenemos esta función como "fallback" de emergencia; la selección real
// viene de /api/wiki y se guarda en state.wiki.zimId.
